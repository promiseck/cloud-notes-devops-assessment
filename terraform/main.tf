...
locals {
  ...
  app_port                  = 8000
  effective_container_image = var.container_image == "" ? "${aws_ecr_repository.app.repository_url}:latest" : var.container_image
  common_tags = {
...
resource "aws_security_group" "db" {
  ...
  description = "Allow PostgreSQL only from app instances"
  vpc_id      = aws_vpc.main.id
...
resource "aws_secretsmanager_secret_version" "app" {
  ...
  secret_string = jsonencode({
    DATABASE_URL = "postgresql://${var.db_username}:${random_password.db.result}@${aws_db_instance.postgres.address}:5432/appdb"
...
resource "aws_launch_template" "app" {
  ...
  user_data = base64encode(templatefile("${path.module}/user_data.sh", {
    aws_region   = var.aws_region
    container    = local.effective_container_image
    secret_id    = aws_secretsmanager_secret.app.id
...
resource "aws_cloudwatch_metric_alarm" "high_cpu" {
  ...
  statistic           = "Average"
  threshold           = 75
  alarm_description   = "CPU is high on the app servers."
  dimensions = {
